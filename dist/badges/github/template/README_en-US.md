<a name="readme-top"></a>

<div align="center">
  <div>
    <img src="https://capsule-render.vercel.app/api?type=waving&color=4D908E&height=160&section=header">
  </div>
  <a href="https://github.com/xiaohuohumax/readme-widget-hub">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=32&pause=1000&width=416&height=68&lines=%F0%9F%8E%96%EF%B8%8FReadme+Widget+Hub%F0%9F%8E%96%EF%B8%8F"/>
  </a>
  <p><b>Use beautiful, configurable badges and status cards to make your README more professional and efficient!</b></p>
  <div>
    <b>
       <a href="/README_en-US.md">Home</a>
      · <a href="README.md">中文</a>
    </b>
  </div>
  <br/>
</div>

## AWESOME Github Stats

**Show everyone how amazing you are! Stay motivated and display your total contributions and level on your GitHub profile README.**

[![ref-repo]](https://github.com/brunobritodev/awesome-github-stats)
[![ref-online-tool]](https://awesome-github-stats.azurewebsites.net/)
[![ref-official-doc]](https://github.com/brunobritodev/awesome-github-stats?tab=readme-ov-file#-options)

### Parameters

<details open>
<summary><small>Click to show/close parameters</small></summary><p></p>

| Name | Type | Required | Default | Description | More Description |
| -------------------- | -------------------- | ------------------------ | ----------------------- | --------------------------- | ------------------------------- |
| ![ref-params] | | | | | |
| `username` | `string` | `true` |  | Github username |  |
| ![ref-querys] | | | | | |
| `cardType` | `string` |  | `level` | Card types | Supported: `level` `level-alternate` `github` `octocat`. |
| `theme` | `string` |  | `default` | Theme | Supported: `default` `dark` `radical` `merko` `gruvbox` and more, see [theme](https://github.com/brunobritodev/awesome-github-stats/blob/master/docs/themes/README.md). |
| `show_icons` | `boolean` |  | `true` | Shown icons at left of labels | Supported: `true` `false`. |
| `preferLogin` | `boolean` |  | `false` | Use github login instead of name | Supported: `true` `false`. |
| `background` | `string` |  |  | Background color | Format: `HEX` `CSS color`, e.g. `FF0000`. |
| `border` | `string` |  |  | Border color | Format: `HEX` `CSS color`, e.g. `FF0000`. |
| `text` | `string` |  |  | Color of the text | Format: `HEX` `CSS color`, e.g. `FF0000`. |
| `title` | `string` |  |  | Color of the title at top | Format: `HEX` `CSS color`, e.g. `FF0000`. |
| `icon` | `string` |  |  | Color of the icons | Format: `HEX` `CSS color`, e.g. `FF0000`. |
| `ring` | `string` |  |  | Color of the ring around the level | Format: `HEX` `CSS color`, e.g. `FF0000`. |

</details>

```txt
https://awesome-github-stats.azurewebsites.net/user-stats/{username}{?cardType,theme,show_icons,preferLogin,background,border,text,title,icon,ring}
```

<p align="right"><a href="#readme-top"><img src="https://img.shields.io/badge/Back%20to%20top-555555?style=for-the-badge"></a></p>

### Examples

```markdown
<!-- brunobritodev -->
![Github Stats Card](https://awesome-github-stats.azurewebsites.net/user-stats/brunobritodev?cardType=level)
```

<div>
  <img src="https://awesome-github-stats.azurewebsites.net/user-stats/brunobritodev?cardType=level" alt="Github Stats Card" />
</div>

<p align="right"><a href="#readme-top"><img src="https://img.shields.io/badge/Back%20to%20top-555555?style=for-the-badge"></a></p>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=4D908E&height=100&section=footer">
</div>

[ref-params]: https://img.shields.io/badge/Path%20Params-526E86

[ref-querys]: https://img.shields.io/badge/Query%20Params-526E86

[ref-repo]: https://img.shields.io/badge/Repository-555555?style=for-the-badge&logo=github

<!-- https://ionic.io/ionicons people-circle-outline -->

[ref-need-register]: https://img.shields.io/badge/Need%20Register-E37945?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons link-outline -->

[ref-online-tool]: https://img.shields.io/badge/Online%20Tools-F94144?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons book-outline -->

[ref-official-doc]: https://img.shields.io/badge/Official%20Doc-277DA1?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons image-outline -->

[ref-static-badge]: https://img.shields.io/badge/Static%20Badge-90BE6D?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons key-outline -->

[ref-need-auth]: https://img.shields.io/badge/Need%20Auth-43AA8B?style=for-the-badge&logo=data:image/svg+xml;base64,
