...
| `mode` | `string` | `true` |  | 模式 | 可选值：`lines`（行模式）`farms`（农场模式）。 |
| ![ref-querys] | | | | | |
| `contribution-view` | `boolean` |  | `true` | 是否在动物头上显示贡献数量。 | 可选值：`true` `false`。 |
...
| -------------------- | -------------------- | ------------------------ | ----------------------- | --------------------------- | ------------------------------- |
| ![ref-params] | | | | | |
| `guildId` | `string` | `true` |  | 公会 ID | 需要去官网[创建公会](https://www.gitanimals.org/en_US/guild/create)获得。 |
