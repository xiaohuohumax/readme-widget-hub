<a name="readme-top"></a>

<div align="center">
  <div>
    <img src="https://capsule-render.vercel.app/api?type=waving&color=4D908E&height=160&section=header">
  </div>
  <a href="https://github.com/xiaohuohumax/badge-collection">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=32&pause=1000&height=68&lines=%F0%9F%8E%96%EF%B8%8FBadge+Collection%F0%9F%8E%96%EF%B8%8F"/>
  </a>
  <p><b>Here&#39;s a curated list of badges used in trending GitHub projects, along with online tools to create them.</b></p>
  <div>
    <b>
       <a href="/README_en-US.md">Home</a>
      · <a href="README.md">中文</a>
    </b>
  </div>
  <br/>
</div>

## Git Animals

**Grow your pets through GitHub activities!**

[![ref-repo]](https://github.com/git-goods/gitanimals)
[![ref-online-tool]](https://www.gitanimals.org/en_US)
[![ref-official-doc]](https://github.com/git-goods/gitanimals?tab=readme-ov-file#%EB%B9%A0%EB%A5%B4%EA%B2%8C-%EC%82%AC%EC%9A%A9%ED%95%98%EA%B8%B0)

### Parameters

#### Line/Farm Mode

<details >
<summary><small>Click to show/close parameters</small></summary><p></p>

| Name | Type | Required | Default | Description | More Description |
| -------------------- | -------------------- | ------------------------ | ----------------------- | --------------------------- | ------------------------------- |
| ![ref-params] | | | | | |
| `username` | `string` | `true` |  | Github username |  |
| `mode` | `string` | `true` |  | Mode | Supported: `lines`(Line Mode) `farms`(Farm Mode). |
| ![ref-querys] | | | | | |
| `contribution-view` | `boolean` |  | `true` | Whether to show the contribution count on the animal's head. | Supported: `true` `false`. |

</details>

```txt
https://render.gitanimals.org/{mode}/{username}
```

#### Guild Mode

<details >
<summary><small>Click to show/close parameters</small></summary><p></p>

| Name | Type | Required | Default | Description | More Description |
| -------------------- | -------------------- | ------------------------ | ----------------------- | --------------------------- | ------------------------------- |
| ![ref-params] | | | | | |
| `guildId` | `string` | `true` |  | Guild ID | Please create a guild on the official website [here](https://www.gitanimals.org/en_US/guild/create). |

</details>

```txt
https://render.gitanimals.org/guilds/{guildId}/draw
```

<p align="right"><a href="#readme-top"><img src="https://img.shields.io/badge/Back%20to%20top-555555?style=for-the-badge"></a></p>

### Examples

#### Line Mode

```markdown
<!-- xiaohuohumax -->
![Git Animals](https://render.gitanimals.org/lines/xiaohuohumax)
```

<div>
  <img src="https://render.gitanimals.org/lines/xiaohuohumax" alt="Git Animals" />
</div>

#### Farm Mode

```markdown
<!-- xiaohuohumax -->
![Git Animals](https://render.gitanimals.org/farms/xiaohuohumax)
```

<div>
  <img src="https://render.gitanimals.org/farms/xiaohuohumax" alt="Git Animals" />
</div>

#### Guild Mode

```markdown
<!-- gitanimals -->
![Git Animals](https://render.gitanimals.org/guilds/668749238833660322/draw)
```

<div>
  <img src="https://render.gitanimals.org/guilds/668749238833660322/draw" alt="Git Animals" />
</div>

<p align="right"><a href="#readme-top"><img src="https://img.shields.io/badge/Back%20to%20top-555555?style=for-the-badge"></a></p>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=4D908E&height=100&section=footer">
</div>

[ref-params]: https://img.shields.io/badge/Path%20Params-526E86

[ref-querys]: https://img.shields.io/badge/Query%20Params-526E86

[ref-repo]: https://img.shields.io/badge/Repository-555555?style=for-the-badge&logo=github

<!-- https://ionic.io/ionicons people-circle-outline -->

[ref-need-register]: https://img.shields.io/badge/Need%20Register-E37945?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons link-outline -->

[ref-online-tool]: https://img.shields.io/badge/Online%20Tools-F94144?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons book-outline -->

[ref-official-doc]: https://img.shields.io/badge/Official%20Doc-277DA1?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons image-outline -->

[ref-static-badge]: https://img.shields.io/badge/Static%20Badge-90BE6D?style=for-the-badge&logo=data:image/svg+xml;base64,

<!-- https://ionic.io/ionicons key-outline -->

[ref-need-auth]: https://img.shields.io/badge/Need%20Auth-43AA8B?style=for-the-badge&logo=data:image/svg+xml;base64,
