...
| `type` | `string` | `true` |  | Badge type | Supported: `v` (latest stable version), `v/unstable` (latest unstable version), `downloads` (total downloads), `license` (license), and more values can be found in [Packagist badges](https://poser.pugx.org/#badges). |
| ![ref-querys] | | | | | |
...
<div>
  <img src="http://poser.pugx.org/phpunit/phpunit/v" alt="Latest Stable Version" />
</div>
